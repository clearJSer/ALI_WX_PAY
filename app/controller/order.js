...
class OrderController extends Controller {
  ...
  async create_order() {
    ...
    const { order_id, order_type, order_price, sign, order_name, redirect_url } = ctx.request.body;
    try {
      if (!order_id) {
        throw 'order_id不为空';
      } else if (!order_type) {
...
const rule = {
      ...
      // 效验客户端传来的数据
      ctx.validate(rule, { order_id});
      const orderStatus = await ctx.model.Orders.find({ where: { order_id } });
	  if (!orderStatus) {
            throw '订单不存在'; 
          }
		const status = orderStatus.pay_status;
       ctx.body = { code: 0, data:status , msg: '' };
    } catch (e) {
      ctx.body = { code: -1, data: '', msg: e };
    }
  }
      ...
      // 查单
 async getorderid() {
    const { ctx, config: { orderValidity } } = this;
    const { order_id } = ctx.query;
    try {
...
const rule = {
      ...
      const orderStatus = await ctx.model.Orders.find({ where: { order_id } });
	   ...
	   if( +new Date(orderStatus.created_at) < (+new Date() - orderValidity * 60 * 1000))throw ('订单已过期!')
        let exire = +new Date(orderStatus.created_at) - ((+new Date()) - orderValidity * 60 * 1000)
	  if (!orderStatus) {
            throw '订单不存在'; 
          }
       ctx.body = { code: 0, data:orderStatus,  time: exire, msg: '订单正常!' };
    } catch (e) {
      ctx.body = { code: -1, data: '', msg: e };
    }
  }
